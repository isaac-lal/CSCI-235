...
{   
    ...
    /*
    this is the loop counting the number of time '.' appears in the url
    since in every working wbesite url there are only two '.'
    */
...
Manual::Manual(std::string title, std::string author, int page_count, std::string device_name, bool is_digital, std::string url, bool has_visual_aid):
        Book(title, author, page_count, is_digital), device_name_{device_name}, has_visual_aid_{has_visual_aid}
        {
            if(!checkFormat(url))
            {
                url_ = "";
